...
import { RoomDetailsBasics } from '../models/room-details-basics';
...
export class CheckAvailabilityComponent implements OnInit {
  ...
  rulesandregulationspath:string;
  ngOnInit(): void {

    this.rulesandregulationspath = window.location.origin + '/rules-and-regulations';
    this.startDate=this._activatedRoute.snapshot.params['startdate']+ ' 13:00';
    this.endDate=this._activatedRoute.snapshot.params['enddate']+ ' 12:00';
    ...
    // this.displayCheckinDate.setFullYear(now.getFullYear());
    // this.displayCheckinDate.setMonth(res.getMonth());
    // this.displayCheckinDate.setDate(res.getDate()+1);

    this.displayCheckinDate=now.toDate();
    this.displayCheckoutDate=now2.toDate();
  ...
  openRulesAndRegulations() {
    const dialogRef = this.dialog.open(RulesandregulationsComponent);
  ...
  openDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result!=undefined)
      {
        if(result.name.length>0 && result.phone.length>0 && (result.email.length>0 && result.email!='NONE'))
        {
          ...
          this.roomDetails.forEach(element => {
            ...
            {
              this.roomDetailsBasics.push({RoomId:element.roomId,IsAcAvailed:element.isAvailingAc,NumberOfPersons:element.numberOfPersons});
            }
